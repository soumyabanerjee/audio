...
{
    if (argc < 3) {
        std::cerr << "Usage: dsdNativePlayer" << " [filename] [deviceName] [alsa_buffer_size] [alsa_period_size]" << std::endl;
        return 1;
    }

    const auto filename = std::string(argv[1]);
    int bufferSize = atoi(argv[3]);
    int periodSize = atoi(argv[4]);
    ...
    std::cout << "DSD Format:" << std::endl
              ...
              << "  ALSA Buffer Size:    " << bufferSize << std::endl
              << "  ALSA Period Size:    " << periodSize << std::endl
    ...
    auto alsa_buffer_size = bufferSize;
    auto blocksize = periodSize;
    ...
    if ((err = snd_pcm_open (&playback_handle, &(std::string(argv[2]))[0], SND_PCM_STREAM_PLAYBACK, 0)) < 0) {
    ...
    if ((err = snd_pcm_prepare (playback_handle)) < 0) {
        std::cerr << "Cannot open audio interface for use" << std::endl;
        goto finalize;
    }
...
bool DSFReader::open(DSDFormat *format)
{
    ...
    _ch = i32;
    if (!_in.read ((char*)&i32, 4) || (i32 != 2822400 && i32 != 5644800 && i32 != 11289600 && i32 != 22579200))
        return false;
